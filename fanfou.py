...
if __name__ == '__main__':
    #print usage
    consumer = {'key':'7593df3904250c63f4bdc32cc0001701','secret':'1a4b347f19ae78cfdb024a101244d0a9'}
    client = XAuth(consumer, '饭否登录名', '饭否登录密码')
    body = {'status': sys.argv[1]}
    client.request('/statuses/update', 'POST',  body)
    #print(client.request('/statuses/public_timeline','GET').read())
    



# 如何使用
# 0. 在饭否创建一个新的app，会得到 consumer key和 secret key（上面的我是乱写了，是假的，哈哈哈）
# 1. 将162行的key和secret换成自己的
# 2. 将163行的饭否登录名和饭否登录密码改成自己的
# 3. 发饭否，在命令行执行，python fanfou.py 我用了20年，才发现自己是个SB
